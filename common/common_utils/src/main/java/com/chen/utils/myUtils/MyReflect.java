package com.chen.utils.myUtils;
...
/**
 * @author Chen
 * @description 反射工具类. 提供调用 getter/setter 方法, 访问私有变量, 调用私有方法
 *              获取泛型类型 Class, 被 AOP 过的真实类等工具函数
 * @create 2021-08-19
 */
@SuppressWarnings("rawtypes")
public class MyReflect {
    // rawtypes 是指传参时也要传递带泛型的参数
    ...
    private static final Logger logger = LoggerFactory.getLogger(MyReflect.class);
            ...
            for (int i = 0; i < cs.length; i++) {
                if (args[i] != null && !args[i].getClass().equals(cs[i])) {
                    if (cs[i] == String.class) {
                        args[i] = ConvertType.toStr(args[i]);
                        if (StringUtils.endsWith((String) args[i], ".0"))
                        {
                            args[i] = StringUtils.substringBefore((String) args[i], ".0");
                        }
                    } else if (cs[i] == Integer.class) {
                        args[i] = ConvertType.toInt(args[i]);
                    } else if (cs[i] == Long.class) {
                        args[i] = ConvertType.toLong(args[i]);
                    } else if (cs[i] == Double.class) {
                        args[i] = ConvertType.toDouble(args[i]);
                    } else if (cs[i] == Float.class) {
                        args[i] = ConvertType.toFloat(args[i]);
                    } else if (cs[i] == Date.class) {
                        if (args[i] instanceof String) {
                            args[i] = MyDate.parseDate(args[i]);
                        } else {
                            args[i] = DateUtil.getJavaDate((Double) args[i]);
                        }
                    } else if (cs[i] == boolean.class || cs[i] == Boolean.class) {
                        args[i] = ConvertType.toBool(args[i]);
                    }
                }
            }
            return (E) method.invoke(obj, args);
